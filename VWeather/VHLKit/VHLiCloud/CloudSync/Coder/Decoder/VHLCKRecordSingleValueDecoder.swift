//
//  VHLCKRecordSingleValueDecoder.swift
//  VHLiCloud
//
...
enum VHLCKRecordSingleValueDecodingError: Error {
    case codingPathMissing
    case unableToDecode
}

final class VHLCKRecordSingleValueDecoder: Decoder {
    ...
    func singleValueContainer() throws -> SingleValueDecodingContainer {
        var container = VHLCKRecordSingleValueDecodingContainer(record: record)
...
struct VHLCKRecordSingleValueDecodingContainer: SingleValueDecodingContainer {
    var codingPath: [CodingKey] = []
    var record: CKRecord
    ...
    func decode<T>(_ type: T.Type) throws -> T where T : Decodable {
        guard let key = codingPath.first else {
            throw VHLCKRecordSingleValueDecodingError.codingPathMissing
        }
        guard let value = record[key.stringValue] as? T else {
            throw VHLCKRecordSingleValueDecodingError.unableToDecode
        }
