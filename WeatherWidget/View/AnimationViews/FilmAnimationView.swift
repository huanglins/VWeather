...
fileprivate struct ProgressShape: Shape {
    let progress: Double
...
struct FilmAnimationImagesView: View {
    var images: [UIImage]
    var duration: TimeInterval
    var body: some View {
...
struct FilmAnimationGIFView: View {
    ...
    var body: some View {
        ...
        if let gifPath = gifPath {
            if let gifData = try? Data(contentsOf: gifPath), let image = UIImage.animatedImage(withGIFData: gifData, cacheKey: gifPath.absoluteString), let images = image.images, images.count > 0 {
                FilmAnimationImagesView(images: images, duration: image.duration)
            }
        }
    }
}
